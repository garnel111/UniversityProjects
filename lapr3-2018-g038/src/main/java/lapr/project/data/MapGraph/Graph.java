...
public class Graph<V,E> implements GraphInterface<V,E> {
    ...
    public boolean validLocation(Location vert) {


        for (Map.Entry<V, Vertex<V, E>> entry : vertices.entrySet()) {

            Location temp = (Location) entry.getKey();
            if (temp.getName().equalsIgnoreCase(vert.getName())) {
                return true;
            }
        }
        return false;
...
int i=0;
        for (Map.Entry<V, Vertex<V, E>> entry : vertices.entrySet()) {
            ...
            Location temp = (Location) entry.getKey();
            if (temp.getName().equalsIgnoreCase(((Location)vert).getName())) {

                Location l = new Location();
                l.setLocation(((Location) entry.getKey()).getLocation());
    ...
    public Edge<V,E> getEdge(V vOrig, V vDest){
        ...
        if (!validLocation(new Location(vOrig)) || !validLocation(new Location(vDest)))
           return null;
    ...
    public Iterable<Edge<V,E>> incomingEdges(V vert){
         ...
         //valida se o vertice é valido.
        if (!validVertex(vert)) 
            return null;
        ArrayList<Edge<V,E>> result = new ArrayList<>();
        //vai percorrer todos os vertices.
        for(Vertex<V,E> vertex : vertices.values()){
            //para cada vertice vai buscar todos os caminhos, usando o getAllOutEdges.
            Edge<V,E> caminho = vertex.getEdge(vert);
            if(caminho != null){
                result.add(caminho);
            }
        }
